...
import com.cic.socket.sender.MeterCurvedSender;
import com.cic.socket.sender.MeterDeviceCreateSender;
import com.cic.socket.sender.MeterDeviceDataByCtdCodeSender;
...
public class SenderFactory {
	 ...
	 * @return
	 */
	public SendingMessage getMeterCurved(String eusCode,String mmpCode){
		SendingMessage  ts = new MeterCurvedSender(serverIP,port,eusCode,mmpCode);
	 ...
	 * xiangx
	 */
	public SendingMessage getMeterDeviceDetailSender(MeterDevice md){
		SendingMessage  ts = new MeterDeviceDetailSender(serverIP,port,md);
	 ...
	 * shuju
	 */
	public SendingMessage getMeterDeviceMeterDataSender(Map<String, String[]> map){
		SendingMessage  ts = new MeterDeviceMeterDataSender(serverIP,port,map);
		return ts;
	}
	/**
	 * 请求计量仪表终端数据
	 ...
	 * shuju
	 */
	public SendingMessage getMeterDeviceDataByCtdCodeSender(String ctdCode){
		SendingMessage  ts = new MeterDeviceDataByCtdCodeSender(serverIP,port,ctdCode);
		return ts;
	}
	/**
	 * 命令 正常
	 ...
	 * @return
	 */
	public SendingMessage getMeterDeviceUpdateSender(String ctdCode,String mmpCode,Double value){
		SendingMessage  ts = new MeterDeviceUpdateSender(serverIP,port,ctdCode,mmpCode,value);
