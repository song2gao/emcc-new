package com.cic.socket.sender;

import java.util.ArrayList;
import java.util.List;

import com.cic.socket.AbstractConnectSender;
import com.cic.pas.common.net.Command;
import com.cic.pas.common.net.Message;
import com.gs.domain.PomsAssembledTerminalDevice;
/**
 * 描述 ：采集器终端详细信息
 * @author wangruibo
 */
public class TerminalDeviceDetailSender extends AbstractConnectSender {
	private List<PomsAssembledTerminalDevice> terminals;
	public TerminalDeviceDetailSender(String ip,int port,List<PomsAssembledTerminalDevice> list){
		super(ip,port);
		this.terminals = list;
	}
	public  List<Message> getMessages(){
		List<Message> messages = new ArrayList<Message>();
		for(PomsAssembledTerminalDevice terminal:terminals){
			Message m = new Message();
			m.setC(Command.CollectStatusDetails);
			m.setCollect(Integer.parseInt(terminal.getAsstdCode()));
			messages.add(m);		
		}
		return messages;
	}
}
