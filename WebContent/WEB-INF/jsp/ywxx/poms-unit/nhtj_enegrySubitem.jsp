...
<html>
	<head>
		<title>用能单位维护-能耗统计配置-用能分项</title>
...
<script type="text/javascript">
	...
	function InitTreeData() {
		$('#MyTree').tree({
			...
			onBeforeExpand : function(node) {
				$('#MyTree').tree('options').url = 'Terminal.action?m=loadTreechild_ynfx&id='+node.id+"&type="+node.attributes.type;
			},
	...
	function GetData1(type,id) {
			...
			args.pageSize = 5;//页容量
			BingData("#tabs_con", args, "Terminal.action?m=load_YnfxTabs", null);
	}   
		...
		$('#tabs_con').datagrid( {
			...
			columns : [ [ {
				...
				onClickRow:function(){  //查询出点击的对象。并把查出的信息放如文本框
					...
					$('#eename').val(row.euEnergyName);
					$('#eeid').val(row.euEnergyId);
					$('#epname').val(row.euParentName);
					$('#epid').val(row.euParentId);
					$('#ename').val(row.euName);
					$('#eorder').val(row.euOrder);
					$('#ebackups').val(row.euBackups);
					$('#ebiaoji').val(row.euBiaoJi);
					$('#eid').val(row.euId);
					$('#ebiaojiId').val(row.euBiaoJiId);
				}			
		});
		GetData1();
	});
$(function() {
	...
	$('#deleteButton').click(function(){
		var row = $('#tabs_con').datagrid('getSelected');
		if(row==null){
			$.messager.alert("提示","选择一个用能分项再进行删除操作!");return false;
		}
		$.messager.confirm('提示','确定要删除"'+row.euName+'"?',function(t){ 
			if(t){
				$.ajax({
		           url: "Terminal.action?m=Del_UsingSubitem",
	...
	$('#saveButton').click(function(){
		...
		var row = $('#tabs_con').datagrid('getSelected');
		if(tree==null && row==null){$.messager.alert("提示","请选择一个用能分项进行增加或更新再保存");return false;}
		...
		$.messager.confirm('提示','确定要更新"'+$('#ename').val()+'"?',function(t){ 
			if(t){
				$('#sysfrom').form({  
				    url:"Terminal.action?m=UpdateOrSave_Subitem",
				    dataType:"json",
				    async:false,
				    onSubmit: function(){  
				        //进行表单验证  
		...
		<div id="main" class="nhtjpz_main">
			...
			<div id="ynxt_right_main">
				...
				<div style="height: 8px;margin-left: -40px;" align="left">
					<ul><li class="licss limov"><input type="button" value="用能分项信息" id="libtn1" class="liimgcss liimgcssmoren"/></ul>
				</div>
				...
				<form id="sysfrom" action="" method="post">
					<table style="width: 800px;margin-left: 20px;margin-top: 10px;">
						...
						<tr>
							<td><font  class="font_cg">系统名称：</font></td>
						...
						<tr>
							...
							<td colspan="3">
								<input id="ebiaoji" name="eBiaoJi" class="BJClass" type="text" />
