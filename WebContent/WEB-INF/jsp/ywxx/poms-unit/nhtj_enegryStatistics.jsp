...
<script type="text/javascript">
	...
	function InitTreeData() {
		$('#MyTree').tree({
			...
			onBeforeExpand : function(node) {
				$('#MyTree').tree('options').url = 'Terminal.action?m=loadTreechild_zdy&id='+node.id+"&type="+node.attributes.type;
			},
	...
	function GetData1(type,id) {
			...
			args.pageSize = 5;//页容量
			BingData("#tabs_con", args, "Terminal.action?m=load_ZdyTabs", null);
	}   
		...
		$('#tabs_con').datagrid( {
			...
			columns : [ [ {
				...
				onClickRow:function(){  //查询出点击的对象。并把查出的信息放如文本框
					...
					$('#eename').val(row.udsEnergyName);
					$('#eeid').val(row.udsEnergyId);
					$('#epname').val(row.udsParentName);
					$('#epid').val(row.udsParentId);
					$('#ename').val(row.udsName);
					$('#eorder').val(row.udsOrder);
					$('#ebackups').val(row.udsBackups);
					$('#ebiaoji').val(row.udsBiaoJi);
					$('#eid').val(row.udsId);
					$('#ebiaojiId').val(row.udsBiaoJiId);
				}			
		});
		GetData1();
	});
$(function() {
	...
	$('#deleteButton').click(function(){
		var row = $('#tabs_con').datagrid('getSelected');
		if(row==null){
			$.messager.alert("提示","选择一个自定义统计再进行删除操作!");return false;
		}
		$.messager.confirm('提示','确定要删除"'+row.udsName+'"?',function(t){ 
			if(t){
				$.ajax({
		           url: "Terminal.action?m=Del_Zdy",
	...
	$('#saveButton').click(function(){
		...
		$.messager.confirm('提示','确定要更新"'+$('#ename').val()+'"?',function(t){ 
			if(t){
				$('#sysfrom').form({  
				    url:"Terminal.action?m=UpdateOrSave_Statistic",
				    dataType:"json",
				    async:false,
				    onSubmit: function(){  
				        //进行表单验证  
		...
		<div id="main" class="nhtjpz_main">
			...
			<div id="ynxt_right_main">
				...
				<div style="height: 8px;margin-left: -40px;" align="left">
					<ul><li class="licss limov"><input type="button" value="自定义系统信息" id="libtn1" class="liimgcss liimgcssmoren"/></ul>
				</div>
				...
				<form id="sysfrom" action="" method="post">
					<table style="width: 800px;margin-left: 20px;margin-top: 10px;">
						...
						<tr>
							<td><font  class="font_cg">自定义名称：</font></td>
