...
<input type="hidden" id="id" name="id" value="<%=request.getAttribute("id") %>"/>
	<table style=" border:1px solid #000" cellspacing="0px" cellpadding="0px">
		<tbody>
			<tr><td style="border-bottom: 1px solid #000"><h3>&nbsp;&nbsp;修改运维信息</h3></td><td style="border-bottom: 1px solid #000"></td></tr>
...
<script type="text/javascript">
	$(function(){
		...
		data:{
			"id":$("#id").val()
				...
				$("#title").attr("value",data.publish.mipTitle);
				editor.ready(function(){
    			 //需要ready后执行，否则可能报错
   				 editor.setContent(data.publish.mipContent);
				})
				$("module").attr("value",data.publish.mipModules);
			}
		});
	});
	...
	$("#saveButton").click(function(){
		...
		$.ajax({
		url: "toInfoPublish.action?m=toUpdateInfoPublish",
		type:"POST",
		dataType:"json",
		timeout: 1000,
		data:{
			...
			"module":$("#module").val() //模块
				...
				if(data.flag == true){
					$.messager.alert("提示","修改成功!","info",function(){window.parent.$('#iframeUpdateDiv').dialog('close'); window.parent.$("#gd").datagrid("loadData",[]); window.parent.GetData();});
				}
				else{
					$.messager.alert("提示","修改失败!","info",function(){window.parent.$('#iframeUpdateDiv').dialog('close'); });
				}
			}
		});
	});
