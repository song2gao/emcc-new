...
<title>用能单位维护——企业信息维护</title>
	...
	<link rel="stylesheet" type="text/css" href="/emcc-new/css/themes/easyui.css">
	<link rel="stylesheet" type="text/css" href="/emcc-new/css/themes/icon.css">
	<link rel="stylesheet" type="text/css" href="/emcc-new/css/themes/demo.css">
...
<script type="text/javascript">
	$(function() {
						...
						.ajax({
							...
							success : function(data) {
								...
								$('#MyCompanyTree')
										.tree(
												{
													url : 'toCodes.action?m=load_CodeTrees&pid='
															+ pid,
													checkbox : false,
													animate : true,
													onBeforeExpand : function(
															node) {
														$('#MyCompanyTree')
																.tree('options').url = 'toCodes.action?m=load_CodeTrees&pid='
																+ node.id;
													},
		...
		$('#saveButton').click(function() {
			...
			var strP = /^\d+$/;
			if ($('#euiCode').val() == "") {
				$.messager.alert('提示', '单位编码不能为空!', 'info');
				return;
			}
			;
			if ($('#euiNames').val() == "") {
				$.messager.alert('提示', '单位名称不能为空!', 'info');
				return;
			}
			;
			if ($('#euiRepresentative').val() == "") {
				$.messager.alert('提示', '法人代表不能为空!', 'info');
				return;
			}
			;
			...
			if ($('#euiContentWay').val() != "") {
				if (strP.test($('#euiContentWay').val()) == false) {
					$.messager.alert('提示', '联系方式请输入数字!', 'info');
					return;
				}
				;
				...
				.tree(
						{
							...
							onBeforeExpand : function(node) {
								alert("点击tree:" + node.id);
								$('#MyTree').tree('options').url = 'tree/treeId&eid='
										+ node.id;
							},
							onClick : function(node) {//单击某个树节点,对应的Tabs发生数据变化
								$("#dataDiv :text")
								...
								$
										.ajax({
											...
											success : function(data) {
												...
												$('#euiConsumeBand')
														.val(
																data.eneUnit.euiConsumeBand);
												$('#euiLocation')
														.val(
																data.eneUnit.euiLocation);
												$('#euiContentWay')
														.val(
																data.eneUnit.euiContentWay);
												$('#euiBuildingArea')
														.val(
																data.eneUnit.euiBuildingArea);
												$('#euiHoldPerson')
														.val(
																data.eneUnit.euiHoldPerson);
												$('#euiChargeDepartment')
														.val(
																data.eneUnit.euiChargeDepartment);
												$('#euiBackups')
														.val(
																data.eneUnit.euiBackups);
...
<body style="font-size: 12px;">
	<div id="main" class="rywh_main"
		style="margin-left: -7px; border: 1px solid;">
		...
		<div id="rywh_right" style="width: 84%;border: 0px solid;">
			...
			<div id="rywh_right_top" align="left">
				<br /> <span style="width: 30px; padding: 25px"> <font
					class="font_cg"></font>
				</span> <input type="button" value="保存" class="btnClass" id="saveForm" />&nbsp;&nbsp;&nbsp;
				<input type="button" id="addForm" value="增加" class="btnClass" /> <input
					id="btnupdate" type="button" value="修改" class="btnClass" /> <input
					type="hidden" id="btnType" value="" /> <input type="button"
					id="deleteButton" value="删除" class="delClass" /> <input
					type="hidden" id="treeid" /> <input type="hidden" id="treename" />
			</div>
			<br />
		</div>
			<!-- 企业信息维护添加页 -->
	<div id="divAdd" class="easyui-panel" title="新建企业信息" style="width:100%;max-width:400px;padding:30px 60px;">
