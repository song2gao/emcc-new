...
(function ($) {
    ...
    }, beforePageText: "第", afterPageText: "页 共{pages}页", displayMsg: "当前显示 {from} - {to} 条记录 共{total} 条记录"
    };
})(jQuery);
(function ($) {
    function _93(_94) {
    ...
    function _102(_103, _104, _105) {
        ...
        if (ul.length) {
            ...
            //change zhouwg
            var _106 = $("<ul></ul>").insertAfter(_104);
...
(function ($) {
    ...
    function _305(_306, _307, _308) {
        ...
        //zhouwg 增加title属性字段，当布局收起的时候需要显示标题
        function _30b(dir,title) {
            var icon;
            if (dir == "east") {
            ...
            var p = $("<div></div>").appendTo(_306).panel({ cls: "layout-expand", title: title, closed: true, doSize: false, tools: [{ iconCls: icon, handler: function () {
                _30f(_306, _307);
                return false;
            } 
            }]
...
(function ($) {
    ...
    function _461(_462) {
        ...
        if (opts.toolbar) {
            if (typeof opts.toolbar == "string") {
                ...
                var tb = $("<div class=\"datagrid-toolbar\"><span class=\"datagrid-toolbar-left\"></span><span class=\"datagrid-toolbar-right\"></span></div>").prependTo(_464);
                for (var i = 0; i < opts.toolbar.length; i++) {
                    ...
                    if (btn == "-") {
                        ...
                        $("<div style=\"float:left;margin:0px 4px 6px 2px;\">"+btn+"</div>").appendTo(tb);
                        }else{
                            $("<div style=\"float:left;margin:6px 4px 6px 2px;\">"+btn+"</div>").appendTo(tb);
                        }
                    }
                    else {
                        ...
                        if(btn.type=='combobox'){
                            tool.css("float","left").appendTo(tb).combobox($.extend({},btn));
                        }else if(btn.type=='menubutton'){
                             tool.css("float","left").appendTo(tb).menubutton($.extend({},btn));
                        }else{
    ...
    function _4b1(_4b2, data) {
        ...
        //todo zhouwg 只有远程排序的时候才需要重新获取page的页数
        if (opts.remoteSort) {
    ...
    function _51e(_51f, _520) {
        ...
        //删除后总条数变动  huang
        if(opts.pagination){
        var paginationOpts=$(_51f).datagrid("getPager").pagination("options");
        if(paginationOpts){
            var total=paginationOpts.total-1;
            $(_51f).datagrid("getPager").pagination("refresh", { total: total });
        }
        }else{
     ...
     var opts = $.data(_525, "datagrid").options;
       ...
       // var view = $.data(_525, "datagrid").options.view;
        ...
        _527.push(_526.row);
          // 新增之后总条数加一 huang
           if(opts.pagination){
         var paginationOpts=$(_525).datagrid("getPager").pagination("options");
        if(paginationOpts){
            var total=paginationOpts.total+1;
           $(_525).datagrid("getPager").pagination("refresh", { total:total });
        }
        }else{
    ...
    var opts = $.data(_529, "datagrid").options;
       ...
       // var view = $.data(_529, "datagrid").options.view;
        ...
        // 新增之后总条数加一 huang
         if(opts.pagination){
         var paginationOpts=$(_529).datagrid("getPager").pagination("options");
        if(paginationOpts){
            var total=paginationOpts.total+1;
           $(_529).datagrid("getPager").pagination("refresh", { total:total });
        }
        }else{
    ...
    $.fn.datagrid.defaults = $.extend({}, $.fn.panel.defaults, { frozenColumns: undefined, columns: undefined, fitColumns: false, autoRowHeight: true, toolbar: null, striped: false, method: "post", nowrap: true, idField: null, url: null, /*loadMsg: "正在获取数据,请稍后...",*/ rownumbers: false, singleSelect: false, selectOnCheck: true, checkOnSelect: true, pagination: false, pagePosition: "bottom", pageNumber: 1, pageSize: 10, pageList: [10, 20, 30, 40, 50], queryParams: {}, sortName: null, sortOrder: "asc", remoteSort: true, showHeader: true, showFooter: false, scrollbarSize: 18, rowStyler: function (_5e5, _5e6) {
